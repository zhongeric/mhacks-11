...
def main():
    ...
    filename = 'adult_svm_model.pkl'
    pickle.dump(cladult, open(filename, 'wb'))
    filename = 'child_svm_model.pkl'
    pickle.dump(clchild, open(filename, 'wb'))
    filename = 'infant_svm_model.pkl'
    pickle.dump(clinfant, open(filename, 'wb'))

    coreml_model = coremltools.converters.svm.convert(cladult)

    coreml_model.short_description = "Predict CPR steps for adults."

    #save the model
    coreml_model.save("adult_cpr.mlmodel")
